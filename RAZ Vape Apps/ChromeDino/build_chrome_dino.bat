...
if defined CHROME_DINO_APP_NAME (
  set APP_NAME=%CHROME_DINO_APP_NAME%
) else (
  set APP_NAME=chrome-dino
)
...
%GCC% %CFLAGS% -c src\main.c -o build\chrome_dino.o || goto :error
...
%GCC% %CPU% -T"..\Shared\n32g031_app.ld" -Wl,--gc-sections %STREAM_LINK_FLAGS% -Wl,-Map=build\%APP_NAME%.map -nostdlib -lnosys ^
  ...
  build\button.o build\battery.o build\nv.o build\app.o %STREAM_OBJECTS% build\chrome_dino.o ^
