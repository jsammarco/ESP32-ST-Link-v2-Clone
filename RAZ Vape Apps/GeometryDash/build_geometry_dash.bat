...
if defined GEOMETRY_DASH_APP_NAME (
  set APP_NAME=%GEOMETRY_DASH_APP_NAME%
) else (
  set APP_NAME=geometry-dash
)
...
%GCC% %CFLAGS% -c src\main.c -o build\geometry_dash.o || goto :error
...
%GCC% %CPU% -T"..\Shared\n32g031_app.ld" -Wl,--gc-sections %STREAM_LINK_FLAGS% -Wl,-Map=build\%APP_NAME%.map -nostdlib -lnosys ^
  ...
  build\button.o build\battery.o build\nv.o build\app.o %STREAM_OBJECTS% build\geometry_dash.o ^
