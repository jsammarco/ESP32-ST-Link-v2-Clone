...
:: Change this to the ST-Link bus ID shown by: usbipd list
set STLINK_BUSID=3-1
:: Use the installed executable directly. This works even from an older CMD
:: window whose PATH was created before usbipd-win was installed or updated.
set "USBIPD=%ProgramFiles%\usbipd-win\usbipd.exe"

if not exist "build\slideshow.bin" (
    echo ERROR: build\slideshow.bin not found. Run build_slideshow.bat first.
    exit /b 1
)
...
rem --cd / prevents WSL from inheriting a Windows current-directory mapping.
wsl.exe --cd / /bin/true >nul 2>&1 || goto :wsl_error
rem Run wslpath inside sh so the Windows path's backslashes are quoted, rather
rem than being consumed as Linux command-line escape characters.
